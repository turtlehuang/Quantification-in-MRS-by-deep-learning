# -*- coding: utf-8 -*-
"""
Created on Sun Dec  1 20:23:06 2019
...
#代謝物濃度
metabo_names = ['Ala','Asp','Cr','PCr','GABA','Glc','Gln','Glu','GPC','PCh','mI','Lac','NAA','NAAG','Scyllo','Tau']#GSH = PCh?
brain_metabos_conc_lower = np.array([0.1,1.0,4.5,3.0,1.0,1.0,3.0,6.0,0.5,0.5,4.0,0.2,7.5,0.5,0,2.0])
brain_metabos_conc_upper = np.array([1.5,2.0,10.5,5.5,2.0,2.0,6.0,12.5,2.0,2.0,9.0,1.0,17,2.5,0,6.0])
...
for steps in tqdm(range(N_specta)):
    ...
    basis_filename = 'gamma_press_te35_123mhz_149.basis'#15 kinds of metabo
    ...
    NDATAB = np.array(SPLIT_BASIS[SPLIT_BASIS.index('NDATAB')+2].split(","))[0].astype(int)
    ...
    for i in range(len(data_idx)):
        if (i <len(data_idx)-1):
            idx = data_idx[i+1]
            meta_bolite_basis = SPLIT_BASIS[idx-NDATAB*2:idx]
    ...
    var_MM_amp = rdm.uniform(float(max_pure_metabo_peak*0.001), float(max_pure_metabo_peak*0.003))
    ...
    np.savez(os.path.join(working_dir,'gamma_gen_folder', f'generate_basis_{steps}'), X=add_noise_crop_filted_sdata, Y=pure_metabo_basis, ppm = crop_ppm)
    np.savez(os.path.join(working_dir,'gamma_gen_folder','other_parameters' ,f'other_parameters_{steps}'), var_MM_amp = var_MM_amp, var_boarden_t2 = var_boarden_t2, var_zero_shift= var_zero_shift, var_AWGN_db = var_AWGN_db)
    brain_betabo_conc_table_df.to_pickle(os.path.join(working_dir,'gamma_gen_folder','brain_betabo_conc_table_df',f'brain_betabo_conc_table_df_{steps}'))
    MM_table_df.to_pickle(os.path.join(working_dir,'gamma_gen_folder','MM_table_df',f'MM_table_df_{steps}'))    
    if steps == 0:
        #np.savez(os.path.join(working_dir,'base_basis_set'),data = base_basis_set)
        np.savez(os.path.join(working_dir,'gamma_42ppm_base_basis_set'),data = base_basis_set)
    #base_basis_set
    '''
