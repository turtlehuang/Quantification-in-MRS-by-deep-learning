...
a = loadmat(r'D:\YuLong_data\hw2\meas_svs.mat')
tdata = a['tdata']
sdata = a['sdata']
...
'''
AU = np.array([0.72,0.28,0.38,0.05,0.45,0.36,0.36,0.04,0.2,0.11,0.64,0.07,1])
cc = np.zeros([len(AU),30000])
for idx,cont in enumerate(AU):
    cc[idx] = np.random.uniform(cont*0.9,cont*1.1,30000)   
    hist, bin_edges = np.histogram(cc[idx])
    plt.figure()
    plt.plot(bin_edges[:-1], hist,'-*')
    plt.ylim(2000,3200)
'''

# 2 ways to generate SNR
'''
import numpy as np
import matplotlib.pyplot as plt
